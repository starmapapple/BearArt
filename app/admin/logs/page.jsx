  ...
  return (
    <AdminShell>
      <div className="page-head">
        <div>
          <h2>操作日志</h2>
          <p>记录后台素材、落地页、订单等关键变更，方便追溯每一次操作。</p>
        </div>
      </div>
      ...
      <section className="card audit-log-panel">
        {logs.length ? (
          logs.map((log) => (
            <article className="audit-log-item" key={log.id}>
              <div className="audit-log-head">
                <div>
                  <span className="pill published">{actionLabel(log.action)}</span>
                  <span className="pill archived">{entityLabel(log.entityType)}</span>
                </div>
          ...
          <p className="muted">暂无操作日志。后台发生变更后会显示在这里。</p>
        )}
      </section>
    </AdminShell>
  );
