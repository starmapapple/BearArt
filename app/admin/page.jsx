  ...
  return (
    <AdminShell>
      <div className="page-head">
        <div>
          <h2>数据概览</h2>
          <p className="muted">查看独立落地页、订单和印尼单品投放漏斗的运行状态。</p>
        </div>
      </div>
      <section className="stats-grid">
        <div className="card">
          <p className="muted">落地页总数</p>
        ...
        <div className="card">
          <p className="muted">订单总数</p>
          <h3>{orders.length}</h3>
        </div>
        <div className="card">
          <p className="muted">已支付收入</p>
      ...
      <section className="admin-dashboard-grid">
        <div className="card admin-alert-card">
          <div>
            <span className="admin-card-kicker">今日重点</span>
            <h3>优先处理 COD 和待付款订单</h3>
            <p className="muted">
              待处理订单 <strong>{pendingOrders}</strong> 个，其中 COD 待确认 <strong>{codPending}</strong> 个。在线支付必须以支付网关
              webhook 为准，成功跳转页不作为真实收款依据。
            </p>
          </div>
        ...
        <div className="card admin-quick-card">
          <span className="admin-card-kicker">常用入口</span>
          <div className="admin-quick-links">
            <Link href="/admin/assets">管理素材</Link>
            <Link href="/admin/products">编辑落地页</Link>
            <Link href="/admin/analytics">查看转化</Link>
            <Link href="/admin/readiness">上线检查</Link>
          </div>
        </div>
        <div className="card admin-quick-card">
          <span className="admin-card-kicker">页面状态</span>
