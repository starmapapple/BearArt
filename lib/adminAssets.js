...
export function getAssetReferences(products = []) {
  ...
  for (const product of products) {
    ...
    if (product.slug === "colorbear-art") {
      if (!product.problemImage) addReference(references, "/uploads/colorbear-art/hero-small.webp", "ColorBear 问题模块图片");
      if (!product.usageGuideImage) addReference(references, "/uploads/colorbear-art/usage-guide-small.webp", "ColorBear 使用说明图片");
      if (!product.gallery?.length) {
        colorBearGalleryFallback.forEach((url) => addReference(references, url, "ColorBear 默认详情图"));
      }
      for (const [reviewer, urls] of Object.entries(colorBearReviewGalleryFallback)) {
        const review = product.reviews?.find((item) => item.name === reviewer);
        if (!review?.gallery?.length) {
          urls.forEach((url) => addReference(references, url, `ColorBear ${reviewer} 评论图片`));
        }
      }
    }
  }
