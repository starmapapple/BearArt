...
export async function getAnalyticsSummary({ orders = [], products = [], days = 30, productSlug = "all" } = {}) {
  ...
  const latestReset = allEvents.find((event) => event.name === "analytics_reset");
  const baseline = latestReset && new Date(latestReset.createdAt) > since ? new Date(latestReset.createdAt) : since;
  const events = allEvents.filter((event) => event.name !== "analytics_reset" && inRange(event.createdAt, baseline) && matchesProduct(event.productSlug, productSlug));
  const scopedOrders = orders.filter((order) => inRange(order.createdAt, baseline) && matchesProduct(order.productSlug, productSlug));
  ...
  const checkoutSubmitters = countUniqueSessionsOrEvents(events, "checkout_submit_attempt");
  ...
  return {
    days: Number(days || 30),
    baselineAt: latestReset?.createdAt || "",
    productSlug,
    products,
    totals: {
    ...
    funnel: [
      ...
      funnelStep("提交尝试", checkoutSubmitters, checkoutOpens),
...
export async function resetAnalyticsBaseline() {
...
function countUniqueSessionsOrEvents(events, name) {
  const matching = events.filter((event) => event.name === name);
  const sessionIds = unique(matching.map((event) => event.sessionId).filter(Boolean));
  return sessionIds.length || matching.length;
}
