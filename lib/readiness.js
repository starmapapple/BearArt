export function getReadinessChecks() {
  const paymentProvider = process.env.PAYMENT_PROVIDER || "mock_gateway";
  const checks = [
    ...
    check("支付 Provider", paymentProvider !== "mock_gateway", "PAYMENT_PROVIDER"),
    check("Webhook Secret", Boolean(process.env.PAYMENT_WEBHOOK_SECRET || process.env.XENDIT_CALLBACK_TOKEN), "PAYMENT_WEBHOOK_SECRET")
  ];
