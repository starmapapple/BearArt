...
export async function createOrder(input) {
  ...
  const allowedPaymentMethods = getEffectivePaymentMethods(product.paymentMethods, process.env.PAYMENT_PROVIDER);
...
export async function updateOrderShipping(id, shippingPatch = {}, options = {}) {
  ...
  const nextShipping = {
    ...
    ...shippingPatch,
    updatedAt: now,
    history: options.addHistory === false
      ? currentShipping.history || []
      : [
          ...(currentShipping.history || []),
          {
            at: now,
            status: shippingPatch.status || currentShipping.status || "updated",
            note: options.note || shippingPatch.status || "shipping_updated"
          }
        ]
  };
  const nextStatus = options.orderStatus || order.status;
  const next = {
    ...order,
...
export async function getOrderByShippingProviderId(providerOrderId) {
  const target = String(providerOrderId || "").trim();
  if (!target) return null;
  const orders = await getOrders();
  return orders.find((order) => order.shipping?.providerOrderId === target) || null;
}
